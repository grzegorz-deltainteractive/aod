...
@section('page_header')
    <div class="container-fluid">
        <h1 class="page-title">
            <img src="/images/gray_laboratoria.png" alt="" class="header-icon-img" /> {{ $dataType->getTranslatedAttribute('display_name_plural') }}
        </h1>
...
@section('content')
    ...
    foreach ($dataTypeContent as $data) {
        ...
        if (isset($data->city) && !empty($data->city)) {
            if (!in_array($data->city, $departments)) {
                $departments[] = $data->city;
            }
        }
   ...
   // $departmentsList = \App\Models\Department::getDepartmentsById($departments);
    $departmentsList = \App\Models\Laboratory::getLaboratoriesById($departments);
                                ...
                                <tbody>
                                        ...
                                        if (!empty($actions)) {
                                            foreach ($actions as $action) {
                                                ...
                                                if ($action->getTitle() == "Pokaż") {
                                                    $pokazUrl = $action->getRoute($dataType->name);
                                                }
                                            }
                                        }
                                    ...
                                    <tr >
                                        ...
                                        <td class="no-sort no-click bread-actions">
                                            @foreach($actions as $action)
                                                @if (!method_exists($action, 'massAction'))
                                                    @include('vendor.voyager.bread.partials.actions', ['action' => $action])
                                                @endif
                                            @endforeach
                                        </td>
                                    </tr>
